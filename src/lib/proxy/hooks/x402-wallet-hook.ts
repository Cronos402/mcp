...
import type { Hook, RequestExtra, ToolCallResponseHookResult } from "cronos402/handler";
...
export class X402WalletHook implements Hook {
    ...
    async processCallToolResult(res: CallToolResult, req: CallToolRequest, extra: RequestExtra): Promise<ToolCallResponseHookResult> {
        try {
            ...
            if (payload.error && payload.error.toLowerCase() === "insufficient_funds") {
                console.log("[X402WalletHook] Insufficient funds detected.");
            ...
            // Cronos402: Users connect their own wallets - no auto-creation
    ...
    private handleInsufficientFunds(res: CallToolResult, payload: X402ErrorPayload): ToolCallResponseHookResult {
        // Cronos402: Direct users to connect their own wallet
        const fundingMessage = `## Funding Required

To make payments on Cronos402:

1. **Connect Your Wallet**: Link your Cronos wallet (MetaMask, Crypto.com Wallet) from your account settings
2. **Get TCRO**: Visit [Cronos Faucet](https://cronos.org/faucet) for testnet CRO
3. **Get devUSDC.e**: Visit [Cronos devUSDC Faucet](https://faucet.cronos.org) for testnet USDC

Ensure you have sufficient CRO or USDC.e balance for the required payment.`;
