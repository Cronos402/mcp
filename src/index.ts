...
import { LoggingHook, withProxy, createBaseMcpHandler, Hook } from "cronos402/handler";
import { AnalyticsHook } from "cronos402/handler";
import { z } from "zod";
...
// Link wallet (manual wallet connection)
app.post("/api/wallets/link", async (c) => {
    console.log('[WALLET LINK] Request received');
    try {
        const session = await auth.api.getSession({ headers: c.req.raw.headers });
        console.log('[WALLET LINK] Session check:', session ? `User ${session.user.id}` : 'No session');
        if (!session) {
            console.log('[WALLET LINK] Unauthorized - no session');
            return c.json({ error: "Unauthorized" }, 401);
        }

        const body = await c.req.json();
        console.log('[WALLET LINK] Request body:', JSON.stringify(body, null, 2));
        const { walletAddress, provider, blockchain, chainId, signature, message } = body;

        if (!walletAddress || !provider || !blockchain || !signature || !message) {
            console.log('[WALLET LINK] Missing required fields');
            return c.json({ error: "Missing required fields" }, 400);
        }

        // Verify signature using viem
        console.log('[WALLET LINK] Verifying signature...');
        ...
        console.log('[WALLET LINK] Signature valid:', isValid);

        if (!isValid) {
            console.log('[WALLET LINK] Invalid signature');
        ...
        if (existingWallet) {
            return c.json({ error: "Wallet already linked to your account" }, 400);
        }

        // Check if wallet is linked to another user
        const walletLinkedToOther = await db.query.userWallets.findFirst({
            where: (t, { eq }) => eq(t.walletAddress, walletAddress),
        });
        ...
        const userWallets = await db.query.userWallets.findMany({
        ...
        console.log(`[WALLET LINK] User ${session.user.id} linked wallet ${walletAddress} on ${blockchain}`);
...
app.get("/api/balance", async (c) => {
    try {
        ...
        let network: CronosNetwork | undefined = undefined;

        if (queryNetwork && isCronosNetwork(queryNetwork)) {
            network = queryNetwork as CronosNetwork;
        }
        ...
        if (!walletAddress) {
            ...
            if (!network && typeof primary.blockchain === "string" && isCronosNetwork(primary.blockchain)) {
                network = primary.blockchain as CronosNetwork;
            }
        }

        // Fallback default network if not provided/derived - use Cronos testnet
        if (!network) {
            network = "cronos-testnet"; // As per Cronos x402 facilitator docs
        }
...
// Payment Routes - handle preflight
app.options("/api/payment/*", (c) => {
    return c.body(null, 204);
});

// Submit signed USDC.e payment to facilitator
app.post("/api/payment/usdc/submit", async (c) => {
    const { submitUsdcPayment } = await import("./lib/api/payment-routes.js");
    return submitUsdcPayment(c);
});

// Check facilitator health
app.get("/api/payment/facilitator/health", async (c) => {
    const { checkFacilitatorHealth } = await import("./lib/api/payment-routes.js");
    return checkFacilitatorHealth(c);
});

// Get supported networks from facilitator
app.get("/api/payment/facilitator/supported", async (c) => {
    const { getSupportedNetworks } = await import("./lib/api/payment-routes.js");
    return getSupportedNetworks(c);
});

// Verify native CRO payment on-chain
app.post("/api/payment/cro/verify", async (c) => {
    const { verifyCroPaymentEndpoint } = await import("./lib/api/payment-routes.js");
    return verifyCroPaymentEndpoint(c);
});
...
app.all("/mcp", async (c) => {
    ...
    const hasTargetUrlHeader = !!c.req.raw.headers.get("x-cronos402-target-url");
    ...
    if (shouldProxy) {
        console.log("[MCP] Proxying request:", {
            ...
            hasTargetUrlHeader: !!original.headers.get("x-cronos402-target-url"),
            targetUrlHeaderValue: original.headers.get("x-cronos402-target-url")?.substring(0, 50) + "...",
        });

        const targetUrl = await resolveTargetUrl(original, c.req.url);
        if (!targetUrl) {
            console.log("[MCP] Failed to resolve target URL. Headers:", {
                "x-cronos402-target-url": original.headers.get("x-cronos402-target-url"),
    ...
    const handler = (session: any) => createBaseMcpHandler(async (server) => {
        server.tool(
            "ping",
